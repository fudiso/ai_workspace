  ...
  const scenarios = [
    {
      ...
      steps: ["요청 접수", "데이터 및 로직 확보", "코드 수정 및 백테스트", "결과 확인 및 저장"]
    },
    {
      id: 2,
      ...
      steps: ["이슈 발생", "자연어 질의", "결과 분석", "자동 알림 발송"]
    },
    {
      id: 3,
      title: "마케팅: 콘텐츠 자동화",
      ...
      steps: ["콘텐츠 작성", "자동화 실행", "프로세스 자동화", "AI검색 연동"]
    },
    {
      id: 4,
      title: "운용 및 리스크: 금융 데이터 리서치",
      department: "운용 및 리스크부서",
      description: "자연어 질의를 통한 복합 금융 데이터 분석 및 자동화",
      detail: "최근 1년간 유로화 환율, 코스피 지수, 해외 주식형 펀드 수익률 데이터를 자연어로 요청하여 상관관계를 분석하고, 매일 자동으로 업데이트되는 리포트를 생성",
      steps: ["데이터 요청", "코드 자동 생성", "탐색 및 분석", "코드/결과 공유", "업무 자동화"]
    },
    {
      id: 5,
      ...
      steps: ["IT 부서의 역할 변화", "현업의 주도적 분석", "자체적 확장", "부서내 공유"]
    },
    {
      id: 6,
      title: "주식운용부문: 자동 펀드 브리핑",
      department: "주식운용부문",
      description: "데이터 적재부터 자동 리포트, AI 분석, 코드 자산화까지 자동화된 펀드 브리핑 제공",
      detail: "PARMS·Dataguide 데이터를 Snowflake로 적재, 워크플로우가 펀드·BM 수익률 및 지분 변동 요약 리포트를 자동 생성해 메일·Teams로 발송. 운용역은 WorkSpace에서 자연어로 추가 분석을 요청하면 LLM이 SQL/Python을 생성·시각화하며, 분석 코드는 코드 카탈로그에 저장 및 공유, 필요시 자동화 스케줄링까지 연계.",
      steps: ["데이터 적재 & 스케줄링", "자동 리포트 수신", "AI 어시스턴트 추가 분석", "코드/결과 자산화"]
    },
    {
      id: 7,
      title: "채권·멀티에셋: 기관 보고서 생성 지원",
      department: "채권·멀티에셋",
      description: "AI 기반 템플릿으로 월간 수익자 보고서 자동 생성 및 발송 지원",
      detail: "운용역이 포털에서 보고서 템플릿을 선택, 파라미터 입력 시 SQL·Python이 각종 지표·차트·테이블을 자동 채우고 Word/PPT로 렌더링. LLM이 운용 코멘트 초안을 제안, 운용역이 편집 후 저장. 완성 파일은 코드 카탈로그와 함께 버전 관리, 워크플로우가 투자기관에 PDF 일괄 발송.",
      steps: ["템플릿 선택", "데이터 자동 채우기", "운용 코멘트 보조", "버전 관리 & 발송"]
    },
    {
      id: 8,
      title: "전략운용본부: 직접 API로 데이터 연구",
      department: "전략운용본부",
      description: "클라우드 IDE와 LLM 어시스턴트로 직접 데이터 분석 및 코드 자산화",
      detail: "운용역이 클라우드 IDE에서 snowflake.connector로 데이터에 직접 접속, LLM 어시스턴트에 자연어로 질의해 Python 코드를 자동 생성·실행. 고급 분석(예: VAR―CVaR 백테스트) 결과와 코드를 코드 카탈로그에 등록, 타 부서와 공유 및 재사용 가능.",
      steps: ["API 게이트 진입", "질문 → 코드 생성", "고급 분석 & 공유"]
    }
  ];
